...
| **A-02** | **ADR-022: Safety kernel (non-bypassable pipeline)** | Fecha 3 CVEs; entry points package-private | `packages/core/src/kernel/index.ts` (new) | 🟡 (2026-04-14, core `SafetyKernel` pronto — permission→safety.in→execute→safety.out→audit, +10 testes, 1203 pass. Falta: wire bridge/tool/provider/cognitive entry points através do kernel + permission persistência via StorageAdapter [S-03]) |
